...
public class TileEntityIndustrialCondenser extends TileEntity implements ITickable, IFluidHandler, ITankPacketAcceptor, INBTPacketReceiver {
    ...
    public TileEntityIndustrialCondenser() {
    ...
    public void update() {
        ...
        if(!world.isRemote) {
            ...
            PacketDispatcher.wrapper.sendToAllAround(new FluidTankPacket(pos, tanks[0]), new NetworkRegistry.TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 150));

            int convert = Math.min(tanks[0].getFluidAmount(), tanks[1].getCapacity() - tanks[1].getFluidAmount())*10;
            if(convert > 0)
                this.waterTimer = 1;
